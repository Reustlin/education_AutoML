...
К сожалению, Streamlit не предоставляет встроенных функций для удаления файлов из папок. Однако, вы можете использовать библиотеки Python для выполнения этой задачи:
...
2. Определите функцию для удаления файлов:
...
3. Добавьте кнопку в Streamlit:

st.button("Удалить файлы", on_click=delete_files_from_folder, args=("path/to/your/folder",))


Объяснение кода:

delete_files_from_folder() - функция, которая принимает путь к папке и удаляет все файлы в этой папке.
os.listdir(folder_path) - возвращает список файлов и папок в указанной папке.
os.path.join(folder_path, filename) - создает полный путь к файлу.
os.path.isfile(file_path) - проверяет, является ли путь к файлу действительным файлом.
os.remove(file_path) - удаляет файл по указанному пути.
st.button(...) - добавляет кнопку в Streamlit. 
on_click=delete_files_from_folder - при нажатии кнопки запускается функция delete_files_from_folder.
args=("path/to/your/folder",) - передает путь к папке в качестве аргумента функции.

Важно:

Замените "path/to/your/folder" на реальный путь к папке, из которой вы хотите удалить файлы.
Прежде чем запускать этот код, убедитесь, что вы понимаете, как работает функция os.remove(). Неправильное использование этой функции может привести к потере данных.
Рекомендуется добавить в приложение механизм подтверждения удаления файлов перед выполнением операции.
Будьте осторожны при использовании этой функции. Она может удалить все файлы в указанной папке, включая важные данные.
